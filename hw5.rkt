...
;; ListOfX -> MUPLList
;; produce a MUPL list from a  Racket list
;; (racketlist->mupllist (list (int 3) (int 4))) ->  (apair (int 3) (apair (int 4) (aunit)))
...
;; MUPLList -> ListOfX
;; produce a Racket list from a MUPL list
;; (mupllist->racketlist (apair (int 3) (apair (int 4) (aunit)))) -> (list (int 3) (int 4))
(define (mupllist->racketlist ms)
  (if (aunit? ms)
      null
      (cons 
       (apair-e1 ms)
       (mupllist->racketlist (apair-e2 ms))))
  )
        ...
        [(ifgreater? e)
         (letrec ([v1 (eval-under-env (ifgreater-e1 e) env)]
           ...
           (if (and (int? v1)
               ...
               (error "MUPL ifgrater applid to non-number")))]
        ;(struct mlet (var e body) #:transparent) ;; a local binding (let var = e in body)
        ;(mlet "x" (int 1) (add (int 5) (var "x"))))
        [(mlet? e)
         (letrec ([var (mlet-var e)]
                  [val (eval-under-env (mlet-e e) env)])
           (eval-under-env (mlet-body e) (cons (cons var val) env)))]
        [(call? e)
         (letrec ([v1 (eval-under-env (call-funexp e) env)]
                  [argval (eval-under-env (call-actual e) env)])
           (if (closure? v1)
               (let ([argname (fun-formal (closure-fun v1))]
                     [fcname (fun-nameopt (closure-fun v1))]
                     [fc (fun-body (closure-fun v1))])
                 (if (string? fcname)
                     (eval-under-env fc (cons (cons argname argval)
                                              (cons (cons fcname fc) 
                                                    (closure-env v1))))
                     (eval-under-env fc (cons (cons argname argval)
                                              (closure-env v1)))))
               (error "bad MUPL expression: ~v" v1)))]
        #; ;sample solution
        [(call? e)
